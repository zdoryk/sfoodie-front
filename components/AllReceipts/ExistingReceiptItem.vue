<template>
  <div
    class="ExistingReceiptItem"
    :class="{
      active: this.activePlan === this.existing_receipt_data.receipt_id,
    }"
    tabindex="0"
...
export default {
  ...
  props: ["existing_receipt_data", "activePlan"],
  methods: {
    ...mapActions(["SELECT_EXISTING_RECEIPT"]),
  ...
  computed: {
    ...
    total_price() {
      let before_sum = JSON.parse(
        JSON.stringify(this.existing_receipt_data.products)
      ).map((item) => item.price);
