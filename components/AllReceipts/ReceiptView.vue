  ...
  <div class="receipt-view">
    ...
    <div id="footer" v-if="isData">
      <div class="receipt-view-total-amount-price">
        <div class="grey total-amount">
          Total {{ total_products_amount }} products
        </div>
        <div class="grey total-price">
          {{ currencySymbol }}{{ total_price }}
        </div>
      </div>
      <red-stroke-button class="delete-button" @click.native="updateVisibility"
        >Delete</red-stroke-button
      >
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    date_format() {
      if (
        ...
        let date = "";
        if (
          typeof this.$store.state.state.selected_receipt.createdAt === "string"
        ) {
          ...
          // console.log(this.$store.state.state.selected_receipt)
          date = new Date(
            this.$store.state.state.selected_receipt.createdAt * 1000
          );
        }
        // const date = new Date(this.receipt[0].createdAt)
        let result =
          date.toLocaleString("en-EG", { month: "short" }) +
          " " +
          date.getDate();
        // console.log(result)
    ...
    total_price() {
      if (
        ...
        let before_sum = JSON.parse(
          JSON.stringify(this.$store.state.state.selected_receipt.products)
        ).map((item) => item.price);
