<template>
  <div id="stacked">
    <client-only>
      <div class="title">Weekday average spending</div>
      <div class="chart">
...
export default {
  ...
  mounted() {
    this.chartOptions.colors = Array.from(
      this.$store.state.state.tree_map_data.colors
    );
    this.$store.watch(
  ...
  methods: {
    ...
    test: function (data, isCategoryData) {
      ...
      if (isCategoryData) {
        ...
        products = Object.fromEntries(
          Object.entries(products).filter(
            ([key, value]) =>
              category_map[key.split(",")[0]] === selected_category
          )
        );
        // this.chartOptions = {...this.chartOptions, ...{
      ...
      productNames = Array.from(
        new Set(Object.keys(products).map((name) => name.split(",")[0]))
      );
      ...
      if (!isCategoryData) {
        ...
        const convertedArray =
          this.$store.state.state.tree_map_data.tree_map_data.map((obj2) => {
      ...
      console.group();
      const convertedArray = JSON.parse(
        JSON.stringify(this.$store.state.state.tree_map_data.tree_map_data)
      ).map((obj) => ({
        name: obj.name,
        data: obj.data.map((item) => item.x),
      }));
      console.log(convertedArray);
      const result = convertedArray
        .find((obj) => obj.name === selected_category)
        .data.map((name) => {
      ...
      if (this.$store.state.state.charts_shared.product.isProductSelected) {
        this.$store.commit(
          "SET_CHART_SELECTED_PRODUCT_NAME",
          result[this.$store.state.state.charts_shared.product.index].name
        );
  ...
  computed: {
    ...
    data_series() {
      ...
      if (isCategory === false) {
        return this.test(
          this.$store.state.state.stacked_bar_data.categories,
          isCategory
        );
      }
      ...
      // }
        ...
        return this.test(
          this.$store.state.state.stacked_bar_data.products,
          isCategory
        );
      }
    },
