  ...
  <div class="tree-map">
    <client-only>
      <blue-button
        class="back-button"
        @click.native="back"
        v-if="this.$store.state.state.charts_shared.isChartCategoryData"
        >< Back</blue-button
      >
...
export default {
  ...
  updated() {
    ...
    let rects = document.querySelectorAll(
      "g.apexcharts-treemap-series .apexcharts-treemap-rect"
    );
    if (back_button) {
      back_button.style.top = rects[0].getBoundingClientRect().top + 10 + "px";
      back_button.style.left =
        rects[0].getBoundingClientRect().left + 10 + "px";
    }
    let index;
    for (const rect of rects) {
      rect.addEventListener("click", () => {
        if (!this.$store.state.state.charts_shared.isChartCategoryData) {
          ...
          this.SET_CATEGORY_CHART_DATA_ACTION({
            selected_category: this.tree_map_data.at(index).name,
          ...
          events: {
            ...
            dataPointSelection: (chartContext, config, { xaxis, yaxis }) => {
              this.product_selection(
                chartContext.target.attributes.selected.value,
                chartContext.target.attributes.j.value
              );
            },
  ...
  methods: {
    ...
    product_selection(isSelected, index) {
      if (this.$store.state.state.charts_shared.isChartCategoryData) {
        ...
        this.$store.commit("SET_IS_CHART_SELECTED_PRODUCT", {
          bool: JSON.parse(isSelected),
    ...
    ...mapActions([
      ...
      "SET_SHARED_CHART_COLORS_ACTION",
      "SET_IS_CHART_SELECTED_PRODUCT_ACTION",
    ]),
    back() {
      this.series = [this.categories_agg];
      const colors = JSON.parse(
        JSON.stringify(this.$store.state.state.tree_map_data.colors)
      );
      this.chartOptions = {
        ...this.chartOptions,
        ...{
