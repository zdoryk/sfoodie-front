  ...
  <div id="email-modal">
    ...
    <div class="content-email">
      <div>
        ...
        <input class="recovery-input" id="email-input" type="email" v-model="email_address" v-on:keyup.enter="send_email">
      </div>
      <blue-button :disabled="!is_email_ok" :style="button_style" class="recovery-button" @click.native="send_email">{{buttonText}}</blue-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async send_email(){
      if (this.is_email_ok){
        try {
          const data = await this.$store.dispatch('SEND_EMAIL_FOR_RECOVERY', this.email_address)
